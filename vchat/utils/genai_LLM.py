...
async def get_ans_from_LLM_Gemini(prompt: str) -> List[str]:
    """
    ...
    model = await genai.GenerativeModel("gemini-1.5-flash")
    ...
    if not check_prompt_is_for_html(response) and not check_prompt_is_for_react(response):
        val = [response, None]
        return val
